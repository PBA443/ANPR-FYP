...
while cap.isOpened():
    ...
    if success:
        ...
        #direction violation
            if 545 < (c_y+offset) and 545 > (c_y-offset) and c_x > 17 and c_x < 1017 :
                go_up[v_id] = c_y
            if v_id in go_up:
                if 470 > (c_y+offset) and (clsid in vehicle_types):
                    cv2.circle(frame, (c_x, c_y), 2, (0, 0, 255), -1)
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 2)
                    cv2.rectangle(frame, (int(x3), int(y3)), (int(x4), int(y4)), (255, 0, 0), 2)
                    vehicle_crop = None
                    if (pc_x > 77 and pc_x < 948 and pc_y < 470 and pc_y > 167):
                        plate_crop = frame[int(y3):int(y4), int(x3):int(x4), :]
                        cv2.imshow('plate', plate_crop)

        #cv2.polylines(frame, [np.array(direction_violation_detection_area, np.int32)], True, (255, 255, 0), 1)
        cv2.line(frame,(17,545), (1017,557), (255, 255, 0), 1)
        cv2.putText(frame, ('line 1'), (17, 545), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 2)
        cv2.line(frame,(77,470), (948,465), (255, 255, 0), 1)
        cv2.putText(frame, ('line 2'), (77,470), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 0), 2)
