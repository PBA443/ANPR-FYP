...
while cap.isOpened():
    ...
    if success:
        ...
        #direction violation
            ...
            if 167 < (c_y + offset) and 167 > (c_y - offset) and c_x > 291 and c_x < 660 and (clsid in vehicle_types):
                down_time[v_id] = time.time()
            if v_id in down_time:
                if 312 < (c_y + offset) and 312 > (c_y - offset):
                    get_time = time.time() - down_time[v_id]
                    distance = 10
                ...
                for val in down_counter_time:
                    for key, value in mj.items():
                        if val == key:
                            sx1, sy1, sx2, sy2, sx3, sy3, sx4, sy4, sclsid = 0, 0, 0, 0, 0, 0, 0, 0, 0
                            sv_id = 0
                            sc_x, sc_y, spc_x, spc_y = 0, 0, 0, 0
                            if len(value) == 5:
                                sx1, sy1, sx2, sy2, sclsid = value
                                sv_id = val
                                sc_x = sx1 + (sx2 - sx1) // 2
                                sc_y = sy1 + (sy2 - sy1) // 2
                            elif len(value) == 9:
                                sx1, sy1, sx2, sy2, sx3, sy3, sx4, sy4, sclsid = value
                                sv_id = val
                                sc_x = sx1 + (sx2 - sx1) // 2
                                sc_y = sy1 + (sy2 - sy1) // 2
                                spc_x = sx3 + (sx3 - sx4) // 2
                                ...
                                # crop license plate
                                license_plate_crop = frame[int(sy3):int(sy4), int(sx3):int(sx4), :]
                                frame_crop = frame[1:596, 1:1019, :]
                                #aaaanpr_area1 = [(1, 1), (1, 596), (1019, 596), (1019, 1)]
                                cv2.imshow("frame2", frame_crop)
                                cv2.imshow("frame", license_plate_crop)
                                license_plate_crop_gray = cv2.cvtColor(license_plate_crop, cv2.COLOR_BGR2GRAY)
                                _, license_plate_crop_thresh = cv2.threshold(license_plate_crop_gray, 64, 255,cv2.THRESH_BINARY_INV)
                                license_plate_text, license_plate_text_score = read_license_plate(license_plate_crop_thresh)
                                print(license_plate_text_score, license_plate_text)
        ...
        #cv2.polylines(frame, [np.array(anpr_area1, np.int32)], True, (255, 255, 0), 1)q

        #die=rection violation detection area
