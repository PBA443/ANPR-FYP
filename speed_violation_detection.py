...
while True:
    ...
    for r in results:
        ...
        for bbox in bbox_id:
            ...
            if cy3 < (cy + offset) and cy3 > (cy - offset):
                go_down[id] = time.time()
            if id in go_down:
                if cy4 < (cy + offset) and cy4 > (cy - offset):
                    end_down_time = time.time() - go_down[id]
                    # cv2.circle(frame, (cx, cy), 2, (0, 0, 255), -1)
                    # cv2.putText(frame, str(id), (x3, y3 - 2), 0, 1, (255, 255, 255), thickness=1, lineType=cv2.LINE_AA)
            ...
            if id in go_up:
                if cy1 < (cy + offset_2) and cy1 > (cy - offset_2):
                    end_up_time = time.time() - go_up[id]
                    # cv2.circle(frame, (cx, cy), 2, (0, 0, 255), -1)
                    # cv2.putText(frame, str(id), (x3, y3 - 2), 0, 1, (255, 255, 255), thickness=1, lineType=cv2.LINE_AA)
    ...
    cv2.putText(frame, ('down side speed = ') + str(down_vehicle_count), (29, 16), cv2.FONT_HERSHEY_PLAIN, 1,
                (255, 255, 255), 2)
    cv2.putText(frame, ('up side   = ') + str(up_vehicle_count), (34, 30), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255),
                2)
