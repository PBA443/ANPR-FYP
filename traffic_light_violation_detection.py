...
def detect_green_color(frame):
    traffic_signal = [(380,74), (380,161), (408,161), (408,74)]
    green = [0, 0, 255]
    plate_crop = frame[74:161, 381:408, :]
...
# output variables
#output_video_path = "dewata_red_light_violation.MP4"
...
red_light_counter = []
up_counter = []
red_light_violation_area = [(277,386), (214,478), (900,497), (775,396)]
vehicle_types = [2]
down = {}
down_counter_direction = []
model = YOLO('yolov8l.pt')
p_model = YOLO('last.pt')
video_path = "dewata_red_light_violation.MP4"
            ...
            if detected_green == True:
                ...
                if red_light_violation_detection >= 0 and conf1 > 60:
                    ...
                    if RLV_plate_crop is not None and not RLV_plate_crop.size == 0:
                        ...
                        print(plate_text)
        cv2.imshow("Red Light Violation", frame)
        #out.write(frame)
